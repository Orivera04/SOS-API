module.exports = {
    ...
    GetAll: (async function(BD, Tipo_Filtro,USUARIO) {
        return new Promise(function(OK, Error) {
            ...
            if(Tipo_Filtro == 1){
                BD.query("SELECT A.*, DATE_FORMAT(A.FECHA_AVERIA,'%d/%m/%y') AS FORMATEADA,TE.NOMBRE_ESPECIALIDAD,U.NOMBRE_USUARIO,U.APELLIDO_USUARIO,EA.DESCRIPCION_ESTADO,A.ID_TIPO , (SELECT concat(NOMBRE_USUARIO," ",APELLIDO_USUARIO) FROM USUARIO WHERE ID_USUARIO = A.ID_MECANICO ) AS MECANICO FROM AVERIA A,USUARIO U,ESTADO_AVERIA EA, TIPO_ESPECIALIDAD TE  WHERE A.ID_USUARIO = U.USUARIO AND EA.ID_ESTADO = A.ID_ESTADO AND A.ID_USUARIO = ? AND TE.ID_ESPECIALIDAD = A.ID_TIPO ORDER BY ID_AVERIA DESC;", [USUARIO],
                    function(Err, Filas) {
                        if (Err) {
                            Error(Err);
                        } else {
