module.exports = {
    ...
    GetAll: (async function(BD, Tipo_Filtro,USUARIO) {
        return new Promise(function(OK, Error) {
            ...
            if(Tipo_Filtro == 1){
                BD.query("SELECT A.*, DATE_FORMAT(A.FECHA_AVERIA,'%d/%m/%y') AS FORMATEADA,TE.NOMBRE_ESPECIALIDAD,U.NOMBRE_USUARIO,U.APELLIDO_USUARIO,EA.DESCRIPCION_ESTADO,A.ID_TIPO  FROM AVERIA A,USUARIO U,ESTADO_AVERIA EA, TIPO_ESPECIALIDAD TE WHERE A.ID_USUARIO = U.USUARIO AND EA.ID_ESTADO = A.ID_ESTADO AND A.ID_USUARIO = ? AND TE.ID_ESPECIALIDAD = A.ID_TIPO;", [USUARIO],
                    function(Err, Filas) {
                        if (Err) {
                            Error(Err);
                        } else {
    ...
    GetByID: (async function(BD, ID) {
        return new Promise(function(OK, Error) {
            BD.query("SELECT A.*,U.NOMBRE_USUARIO,U.APELLIDO_USUARIO,U.TELEFONO_USUARIO ,UC.* ,EA.DESCRIPCION_ESTADO FROM AVERIA A, USUARIO U , USUARIO_CARRO UC, ESTADO_AVERIA EA WHERE A.ID_AVERIA = ? AND U.USUARIO = A.ID_USUARIO AND UC.ID_USUARIO = A.ID_USUARIO AND EA.ID_ESTADO = A.ID_ESTADO;", [ID],
                function(Err, Filas) {
                    if (Err) {
                        Error(Err);
                    } else {
